...
1. An **exponential decay filter** (also called exponential smoothing) is a rule of thumb method to filter out noise in time series signals. It can be implemented by the following equation:
   ...
   where S is the filtered signal, x is the original signal and α is the exponential factor. For more details, please see [Wikipedia - Exponential smoothing](https://en.wikipedia.org/wiki/Exponential_smoothing).
...
1. Do some exercises, such as jumping jacks. Then take measurement again and estimate your heart rate, by repeating the previous steps. You need to be quick as your heart rate may recover to baseline quickly.
